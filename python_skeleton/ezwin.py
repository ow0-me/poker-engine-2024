...
class Player(Bot):
    ...
    def get_action(self, observation: dict) -> Action:
        ...
        minraiseodds = (observation["min_raise"]) / (pot_size + observation["min_raise"])
        #print(continue_cost, observation["min_raise"])
        ...
        if continue_cost > 1:
            equity = (equity - 0.2) / 0.8
            self.log.append(f"Adjusted equity: {equity}")

        if equity > 0.99 and RaiseAction in observation["legal_actions"]:
            raise_amount = observation["max_raise"]
            action = RaiseAction(raise_amount)
        elif RaiseAction in observation["legal_actions"] and equity >= 0.99:
